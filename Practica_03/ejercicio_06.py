...
class ShoppingCart:
    ...
    # Test de Conversión a string: Al momento de convertir el carrito de compras a str sucede esto 

    def __str__(self): 

        print(f"{[str(art) for art in self.articles]}") #así
        print(f"{[art.name for art in self.articles]}") #otra forma

        return f"{[str(art) for art in self.articles]}"
